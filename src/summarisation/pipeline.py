...
from src.summarisation.providers import OllamaSummariser, GeminiSummariser
...
class SummarisationPipeline:
    ...
    async def generate_document_summary(self, doc: Document) -> str:
        ...
        console.print(f"[dim]Generating document summary for {doc.source_file}...[/dim]")
    ...
    async def generate_section_summaries(self, chunks: List[Chunk], doc_summary: str) -> Dict[str, str]:
        if not self.enabled:
            return {}
        ...
        for chunk in chunks:
            if chunk.source_metadata.section:
                sections.setdefault(chunk.source_metadata.section, []).append(chunk.text)
        ...
        for section, texts in sections.items():
            ...
            try:
                ...
                console.print(f"[dim]Generating section summary for: {safe_section}[/dim]")
                ...
                console.print(f"[red]Error generating section summary: {e}[/red]")
