...
class DynamicLLMCleaner:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if self.provider == 'gemini':
            ...
            self.gemini_model = genai.GenerativeModel(self.params.get('model', 'gemini-1.5-flash'))
        elif self.provider == 'ollama':
            from ollama import AsyncClient
            self.ollama_client = AsyncClient(host=self.params.get('url', 'http://localhost:11434'))
    ...
    async def _identify_patterns_with_llm(self, sample: str, filename: str) -> List[Dict[str, Any]]:
        prompt = f"""
...
"""
        try:
            ...
            if isinstance(parsed, dict):
                # The LLM might have wrapped the array in an object like {"patterns": [...]}
                for key in parsed:
            ...
            console.print(f"[red]LLM Generation failed or invalid JSON: {e}[/red]")
    ...
    async def clean(self, text: str, filename: str) -> Tuple[str, List[Dict]]:
        ...
        console.print(f"[dim]Requesting dynamic LLM cleaner patterns for {filename}...[/dim]")
        ...
        for rule in patterns:
            ...
            try:
                ...
                console.print(f"[green]Stripped pattern: '{pattern_str}' ({blast_percentage:.3f}% removed)[/green]")
