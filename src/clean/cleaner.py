...
class DocumentCleaner:
    ...
    def _deduplicate_consecutive_paragraphs(self, text: str) -> str:
        ...
        for p in paragraphs:
            ...
            # Max sequence length is 5 to prevent O(N^3) catastrophic freezing on large documents
            max_seq = min(5, len(deduplicated) // 2)
    ...
    def clean(self, text: str) -> str:
        ...
        if self.config.regex_removals:
            for pattern in self.config.regex_removals:
                try:
                    # Use re.IGNORECASE by default for user convenience, unless
                    # specified otherwise
                    cleaned_text = re.sub(
                        pattern, '', cleaned_text, flags=re.MULTILINE)
                except re.error as e:
                    print(f"Warning: Invalid regex pattern '{pattern}' - {e}")

        # 4. Collapse consecutive newlines (more than 2 into exactly 2)
        if self.config.collapse_newlines:
            # Replaces 3 or more newlines with exactly 2 newlines (a standard
            # paragraph break)
            cleaned_text = re.sub(r'\n{3,}', '\n\n', cleaned_text)

            # Replaces multiple empty spaces on a line with a single space, but leaves indentation alone
            # Actually, let's just stick to newline collapsing to avoid ruining
            # markdown tables.

        # 5. Deduplicate consecutive paragraphs
        if self.config.deduplicate_paragraphs:
            cleaned_text = self._deduplicate_consecutive_paragraphs(
                cleaned_text)
