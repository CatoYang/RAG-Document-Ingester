...
class PresentationExtractor(BaseExtractor):
    ...
    def extract(self, file_path: str, **kwargs) -> Document:
        ...
        console.print(
            f"[cyan]Extracting Presentation {file_path_obj.name}...[/cyan]")

        try:
            # MarkItDown natively parses PPTX, extracting both slide text and
            # speaker notes.
            result = self.md.convert(file_path)
        except Exception as e:
            console.print(
                f"[red]Error parsing presentation {file_path_obj.name}: {e}[/red]")
            return Document(
