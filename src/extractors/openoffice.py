...
class OpenOfficeExtractor(BaseExtractor):
    ...
    def __init__(self):
        ...
        if not shutil.which("libreoffice"):
            raise RuntimeError("LibreOffice is not installed or not in PATH. Please run `sudo apt install libreoffice`.")
    ...
    def extract(self, file_path: str, **kwargs) -> Document:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            console.print(f"[cyan]Converting {file_path_obj.name} to .docx via headless LibreOffice...[/cyan]")
            
            # Run the headless conversion
            # Syntax: libreoffice --headless --convert-to docx <file> --outdir <dir>
            try:
                subprocess.run(
                    [
                        "libreoffice", 
            ...
            if not converted_files:
                raise FileNotFoundError(f"LibreOffice succeeded but no .docx file was found in {temp_dir_path}")
                
            converted_file_path = converted_files[0]
            
            # Pass the converted file to MarkItDown
            console.print(f"[cyan]Extracting Markdown from converted file using MarkItDown...[/cyan]")
            # We bypass the MarkItDownExtractor's standard extract() to avoid nested Documents,
            # or we can just call it and merge metadata. Let's call it and overwrite the source.
            sub_doc = self.markitdown_extractor.extract(str(converted_file_path))
