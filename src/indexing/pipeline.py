...
class IndexingPipeline:
    ...
    async def initialize(self, config_path: str = "config/config.yaml"):
        ...
        chunker_cls = get_chunker_class(
            chunker_cfg.get('type', 'MarkdownChunker'))
        self.chunker = chunker_cls(**chunker_cfg.get('params', {}))

        embedder_cls = get_embedder_class(
            embedder_cfg.get('type', 'OllamaEmbedder'))
    ...
    async def process_file(self, file_path: str):
        ...
        try:
            ...
            if not chunks:
                console.print(
                    f"[yellow]Warning: No chunks generated for {filename}[/yellow]")
                return
            ...
            if doc_summary:
                ...
                doc_meta = SourceMetadata(
                    filename=doc.source_file, chunk_index=-1)
                doc_chunk = Chunk(
                    text=doc_summary,
            ...
            if section_summaries:
                ...
                for sec, summary in section_summaries.items():
                    sec_meta = SourceMetadata(
                        filename=doc.source_file, chunk_index=-2, section=sec)
                    sec_chunks.append(
                        Chunk(
                            text=summary,
                            source_metadata=sec_meta,
                            metadata={
                                "level": "section",
                                "source": filename}))
    ...
    async def process_directory(self, directory: str):
        ...
        console.print(
            f"[bold green]Starting Indexing Pipeline on {dir_path}[/bold green]")

        if not dir_path.exists():
            console.print(
                f"[bold red]Directory {dir_path} does not exist.[/bold red]")
            return
