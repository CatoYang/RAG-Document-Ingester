...
- [x] **Carry `page_number` through chunking** — `MarkdownChunker._consume_page_markers` now parses `PyMuPDF4LLMExtractor`'s page markers out of each chunk's text (stripping them from the stored/embedded text), assigning the last page number seen so far to that chunk and carrying it forward for chunks that fall between two markers. Extractors that don't emit page markers still work; `page_number` just stays `None` for them, as before. Fixed 2026-09-15, covered by `tests/test_chunkers.py`. Known quirk (pre-existing, not introduced by this): `MarkdownChunker` can emit an empty leading chunk when content starts before the first header - worth filtering out before embedding (see Path 2).
- [ ] **Pull `nomic-embed-text`** on the Ollama host (only `llama3`, `llava`, `minicpm-v` are currently pulled — see `CLAUDE.md`). Per CLAUDE.md's rule, needs to be confirmed with the user before pulling (technically possible to trigger remotely via `POST http://<gateway-ip>:11434/api/pull` without touching the Windows side directly, but it's still a multi-GB download onto their machine).
...
- [ ] **Actually run this against the real corpus** — none of the above has been run end-to-end against `data/raw` yet (validated with unit tests + one real single-file, no-disk-write smoke test on a 1-page PDF only). Needs, in order: `nomic-embed-text` pulled (above), then `python main.py config/config_text_corpus.yaml` (ask first — see `CLAUDE.md`, this will eventually reach the 10 scanned PDFs and load Marker), then a filled-in golden Q&A set, then `python eval_golden.py config/config_text_corpus.yaml eval/golden_qa.yaml` for a baseline recall number.
...
- [ ] **Drop empty/near-empty chunks before embedding** — `MarkdownChunker` can emit an empty leading chunk when a document's content starts before its first header (observed on real corpus content, 2026-09-15); wastes an embed call and adds a useless point to the vector store.
